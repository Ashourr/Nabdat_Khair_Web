...
export default function HomeVolunteer() {
  ...
  const opportunities = [
    {
      id: 1,
      title: "قافلة إطعام - حي الأسمرات",
      org: "بنك الطعام المصري",
      loc: "القاهرة",
    },
    {
      id: 2,
      title: "تصميم جرافيك للمنصة",
      org: "مؤسسة نبضة خير",
      loc: "عن بعد",
    },
  ];
  ...
  return (
    <div className="volunteer-dashboard">
      <div className="title">
        ...
        <p>
          إليك نظرة سريعة على نشاطك التطوعي وأحدث الفرص المتاحة لخدمة المجتمع.
        </p>
      </div>
            ...
            <div className="opp-list">
              {opportunities.map((opp) => (
                <div className="opp-card" key={opp.id}>
                  ...
                  <Link
                    href={`/${locale}/dashboard/volunteer/opportunities/${opp.id}`}
                    className="btn-apply"
                  >
                    تقديم الآن
                  </Link>
                ...
                <tbody>
                  {tasks.map((task, index) => (
                    <tr key={index}>
                      ...
                      <td>
                        <span
                          className={`status-badge ${task.status === "مؤكد" ? "success" : "pending"}`}
                        >
                          {task.status}
                        </span>
                      </td>
                      <td>
                        <Link
                          href={`/${locale}/dashboard/volunteer/myTasks/${task.taskName}`}
                        >
                          التفاصيل
                        </Link>
                      </td>
